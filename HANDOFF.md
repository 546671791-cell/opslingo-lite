...
当前版本：`1.4.0`。这是零账号、零跟踪的 Preact PWA；场景与词句数据写入当前设备的 IndexedDB。网页与 iPhone 继续使用离线参考音或已授权的在线美式神经语音；Android APK 内置量化 Kokoro 离线神经美式语音，可在“自动 / 在线高质量 / 离线神经美式 / 系统兼容”间切换，离线播放不上传文本。Android 神经推理运行在独立语音进程中，设备不兼容或语音进程异常时主界面不会退出，并会切换为系统朗读。句子与对话会在内容更新、应用启动及进入课程前预取；词汇可按包下载在线自然语音到当前设备，发音评分也是经用户确认后启用的可选服务。
