...
当前版本：`1.2.3`。这是零账号、零跟踪的 Preact PWA；场景与词句数据写入当前设备的 IndexedDB。默认使用离线参考音；配置 Azure Speech 后，所有英语播放优先使用美式神经语音，核心句子和对话会在内容更新、应用启动及进入课程前预取到当前设备，发音评分也是经用户确认后启用的可选服务。
